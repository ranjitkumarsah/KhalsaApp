        ...
        <div class="row">
          <div class="col-12">
            <div class="card">
              <!-- /.card-header -->
              <div class="card-body" >
                <a href="{{url('Subadmin/add_cardfamily')}}/{{$id}}" class="btn btn-primary" style="float:right;">Add CardHolderFamily</a><br><br>
                <table  id="Table_ID" class="table table-responsive-sm table-striped table-bordered table-hover dataTable no-footer" >
                  ...
                  <?php $i=1; ?>
                    ...
                    <td>{{$i++}}</td> 
                      <td>{{$data->fname}}</td>                    
                      <td>{{$data->card_id}}</td>                    
                      <td>{{$data->famritdhari}}</td>                     
                      <td>{{$data->fage}}</td>
                      <td>{{$data->faadhaar}}</td> 
                      <td>{{$data->fqualification}}</td>
                      <td>{{$data->fjob}}</td>
                      <td>{{$data->fsalary}}</td> 
                      <td>{{$data->frelation}}</td> 
                      <td>
                        <a href="{{url('Subadmin/editcardholderfamily')}}/{{$data->id}}"><i class="fa-solid fa-square-pen" style="font-size:25px;"></i></a>
                        <a href="{{url('Subadmin/deletecardholderfamily',$data->id)}}"><i class="fa-solid fa-trash-can" style="color:red; font-size:23px;" onclick="return confirm('Are you sure to delete this?')"></i></a>
                      </td>
                    </tr>
                    @endforeach                  
                  </tbody>
